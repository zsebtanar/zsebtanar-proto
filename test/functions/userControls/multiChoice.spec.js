import multiChoice from '../../../functions/src/userControls/multiChoice'
...
describe('multiChoice', () => {
  ...
  it('should fail if no solution', () => {
    [null, undefined, {}, []].map(input =>
      expect(multiChoice(null, input, input)).toBe(false)
    )
  })

  it('should fail if the user input empty', () => {
    [null, undefined, {}].map(input =>
      expect(multiChoice(null, {idA: true}, input)).toBe(false)
    )
  })
})
